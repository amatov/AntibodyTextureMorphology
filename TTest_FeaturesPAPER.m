...
% if i == 2 % MKN7 resistant ONE
%          figure, boxplot([CTL_feats(:,6),DTX_feats(:,6)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'6','6D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #6');
%     title(cellline{i})
%      
% %             figure, boxplot([CTL_feats(:,13),DTX_feats(:,13)],'notch','on','whisker',1.5,  ...
% %         'widths', 0.8, 'labels', {'13','13D'},...
% %          'positions' , [ 1,3])
% % %        ylim([-1 15])
% %     ylabel('only feat #13');
% %     title(cellline{i})
% end
% if i == 7 % HS resistant TWO
%         figure, boxplot([CTL_feats(:,9),DTX_feats(:,9)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'9','9D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #9');
%         title(cellline{i})
% 
%             figure, boxplot([CTL_feats(:,11),DTX_feats(:,11)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'11','11D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #11');
%     title(cellline{i})
% end
% if i == 6 % SCH resistant THREE
% 
% 
%             figure, boxplot([CTL_feats(:,11),DTX_feats(:,11)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'11','11D'},...
...
%     title(cellline{i})
%     
%             figure, boxplot([CTL_feats(:,20),DTX_feats(:,20)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'20','20D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #20');
%         title(cellline{i})
%                     figure, boxplot([CTL_feats(:,21),DTX_feats(:,21)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'21','21D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #21');
%         title(cellline{i})
% end
% if i == 3 % SNU sensitive FOUR
%         figure, boxplot([CTL_feats(:,9),DTX_feats(:,9)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'9','9D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
...
%     ylabel('only feat #11');
%     title(cellline{i})
%                 figure, boxplot([CTL_feats(:,12),DTX_feats(:,12)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'12','12D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #12');
%     title(cellline{i})
...
%         title(cellline{i})
% 
%             figure, boxplot([CTL_feats(:,14),DTX_feats(:,14)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'14','14D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #14');
%     title(cellline{i})
%                 figure, boxplot([CTL_feats(:,15),DTX_feats(:,15)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'15','15D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #15');
%     title(cellline{i})
%                 figure, boxplot([CTL_feats(:,19),DTX_feats(:,19)],'notch','on','whisker',1.5,  ...
%         'widths', 0.8, 'labels', {'19','19D'},...
%          'positions' , [ 1,3])
% %        ylim([-1 15])
%     ylabel('only feat #19');
%     title(cellline{i})
...
if i == 1 % TMK1 sensitive TEN
            figure, boxplot([CTL_feats(:,2),DTX_feats(:,2)],'notch','on','whisker',1.5,  ...
        'widths', 0.8, 'labels', {'2','2D'},...
         'positions' , [ 1,3])
%        ylim([-1 15])
    ylabel('only feat #2');
        title(cellline{i})

            figure, boxplot([CTL_feats(:,3),DTX_feats(:,3)],'notch','on','whisker',1.5,  ...
        'widths', 0.8, 'labels', {'3','3D'},...
         'positions' , [ 1,3])
%        ylim([-1 15])
    ylabel('only feat #3');
    title(cellline{i})
                figure, boxplot([CTL_feats(:,4),DTX_feats(:,4)],'notch','on','whisker',1.5,  ...
        'widths', 0.8, 'labels', {'4','4D'},...
         'positions' , [ 1,3])
%        ylim([-1 15])
    ylabel('only feat #4');
    title(cellline{i})
                figure, boxplot([CTL_feats(:,7),DTX_feats(:,7)],'notch','on','whisker',1.5,  ...
        'widths', 0.8, 'labels', {'7','7D'},...
         'positions' , [ 1,3])
%        ylim([-1 15])
    ylabel('only feat #7');
    title(cellline{i})
                    figure, boxplot([CTL_feats(:,10),DTX_feats(:,10)],'notch','on','whisker',1.5,  ...
        'widths', 0.8, 'labels', {'10','10D'},...
         'positions' , [ 1,3])
%        ylim([-1 15])
    ylabel('only feat #10');
    title(cellline{i})
